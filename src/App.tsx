    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-blue-600/30 overflow-x-hidden">
      ...
      <main className={`transition-all duration-300 ${activeTool ? 'pt-16' : 'pt-24 pb-20 px-4 md:px-6 max-w-6xl mx-auto'}`}>
        {!activeTool ? (
          ...
          /* FULL SCREEN CHAT INTERFACE - Gemini Style */
          <div className="flex flex-col h-[calc(100vh-64px)] w-full bg-[#050505] animate-in slide-in-from-right-4">
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 flex flex-col items-center">
               <div className="w-full max-w-3xl space-y-6">
                ...
                {isLoading && <div className="text-blue-500 text-[10px] font-black italic animate-pulse px-6 uppercase tracking-[0.2em]">Toolscout AI is typing...</div>}
                <div ref={chatEndRef} />
               </div>
            </div>

            {/* Chat Input Area - Fixed at bottom */}
            <div className="p-4 md:p-8 bg-gradient-to-t from-[#050505] via-[#050505] to-transparent flex justify-center">
              <form onSubmit={handleSendMessage} className="relative w-full max-w-3xl flex items-center gap-3">
                <div className="relative flex-1 group">
                  <input 
                    ...
                    className="w-full bg-[#121212] border border-white/10 rounded-3xl py-5 px-8 focus:outline-none focus:border-blue-600/50 text-sm text-white shadow-2xl transition-all" 
                  />
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
                    <button type="submit" className="p-3 bg-blue-600 rounded-2xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20 active:scale-90">
                      <Send size={20} />
                    </button>
                  </div>
                </div>
